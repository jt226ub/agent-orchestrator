...
func TestExclusiveAdmissionClosesSharedLaneBeforeDrain(t *testing.T) {
	...
	if _, err := gate.AcquireShared(context.Background()); !errors.Is(err, ports.ErrAgyAccountSwitchInProgress) {
...
func TestRetainedExclusiveLeaseBlocksUntilExplicitRelease(t *testing.T) {
	...
	if _, err := gate.AcquireExclusive(context.Background()); !errors.Is(err, ports.ErrAgyAccountSwitchInProgress) {
