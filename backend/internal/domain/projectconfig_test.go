...
func TestProjectConfigValidate(t *testing.T) {
	tests := []struct {
		...
		{"profile interface tui", ProjectConfig{Profiles: map[string]RoleProfile{"p": {Interface: SessionModeTUI}}}, false},
		{"profile interface chat", ProjectConfig{Profiles: map[string]RoleProfile{"p": {Interface: SessionModeChat}}}, false},
		{"profile unknown interface", ProjectConfig{Profiles: map[string]RoleProfile{"p": {Interface: "terminal"}}}, true},
		{"profile bad agent config", ProjectConfig{Profiles: map[string]RoleProfile{"p": {AgentConfig: AgentConfig{Permissions: "yolo"}}}}, true},
		{"good quota and fallback", ProjectConfig{Profiles: map[string]RoleProfile{"flash": {Harness: HarnessAgy, Quota: &ProfileQuota{WarnBelowPercent: 20, RefuseBelowPercent: 5}, Fallback: &ProfileFallback{Profile: "paid"}}, "paid": {Harness: HarnessClaudeCode}}}, false},
		{"quota refuse only", ProjectConfig{Profiles: map[string]RoleProfile{"flash": {Quota: &ProfileQuota{RefuseBelowPercent: 10}}}}, false},
		{"quota above 100", ProjectConfig{Profiles: map[string]RoleProfile{"flash": {Quota: &ProfileQuota{WarnBelowPercent: 101}}}}, true},
		{"quota negative", ProjectConfig{Profiles: map[string]RoleProfile{"flash": {Quota: &ProfileQuota{RefuseBelowPercent: -1}}}}, true},
		{"quota refuse above warn", ProjectConfig{Profiles: map[string]RoleProfile{"flash": {Quota: &ProfileQuota{WarnBelowPercent: 10, RefuseBelowPercent: 20}}}}, true},
		{"fallback without quota", ProjectConfig{Profiles: map[string]RoleProfile{"flash": {Fallback: &ProfileFallback{Profile: "paid"}}, "paid": {}}}, true},
		{"fallback to self", ProjectConfig{Profiles: map[string]RoleProfile{"flash": {Quota: &ProfileQuota{}, Fallback: &ProfileFallback{Profile: "flash"}}}}, true},
		{"fallback to unknown profile", ProjectConfig{Profiles: map[string]RoleProfile{"flash": {Quota: &ProfileQuota{}, Fallback: &ProfileFallback{Profile: "missing"}}}}, true},
