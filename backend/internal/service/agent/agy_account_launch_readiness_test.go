...
const otherAgyAccountID = "bb1e9a5d-37ad-43f8-83bd-13de8168f8af"

// agyLaunchReadinessFixture reproduces the important protocol distinction:
...
type agyLaunchReadinessFixture struct {
	t                        *testing.T
	manager                  *agyAccountManager
	service                  *Service
	active                   agyAccountRecord
	other                    agyAccountRecord
	mu                       sync.Mutex
...
type agyReadCall struct {
	managed bool
	refresh bool
}

func newAgyLaunchReadinessFixture(t *testing.T) *agyLaunchReadinessFixture {
	t.Helper()
	root := t.TempDir()
	...
	state := &testAgyDeviceSeed{active: testAgyDeviceAccount{AccountID: agyTestAccountID, Revision: 1}}
	...
	ids := []string{agyTestAccountID, otherAgyAccountID, "6f8dfc76-8db4-4621-8974-c480093e0d55"}
	...
	active := agyCommitTestAccount(t, manager.catalog, manager.pendingRoot, "b60a377d-da68-4a61-86f2-f31f04c571f2", ports.AgyAccountObservation{
		Authentication: domain.AgentAuthenticationAuthorized, Method: domain.AgyAuthMethodGoogle, Email: &activeEmail,
	})
	other := agyCommitTestAccount(t, manager.catalog, manager.pendingRoot, "1c5de3ab-82d0-4a68-a06b-8495cdeab909", ports.AgyAccountObservation{
		Authentication: domain.AgentAuthenticationAuthorized, Method: domain.AgyAuthMethodGoogle, Email: &otherEmail,
	})
	activeCredential, err := readOpaqueCredential(filepath.Join(active.Home, agyCredentialFilename))
	if err != nil {
		t.Fatal(err)
	}
	// The active account is backed by the device-global Agy home.
	...
	manager.reconciliation = domain.AgyDeviceReconciliation{Status: domain.AgyDeviceReconciliationVerified, ActiveAccountVerified: true, ReasonCode: "verified"}
	manager.deviceAccountID = active.Snapshot.ID
	manager.deviceCredentialPresent = true
	fixture := &agyLaunchReadinessFixture{t: t, manager: manager, active: active, other: other}
	manager.factory = &fakeAgyAccountFactory{capabilities: supportedAgyAccountCapabilities(), open: func(account ports.AgyAccountContext) (ports.AgyAccountClient, error) {
		return &fakeAgyAccountClient{readFn: func(_ context.Context) (ports.AgyAccountObservation, error) {
			...
			refresh := false
			fixture.mu.Lock()
			fixture.reads = append(fixture.reads, agyReadCall{managed: account.Managed, refresh: refresh})
			refreshAuthorized := fixture.refreshReturnsAuthorized
			fixture.mu.Unlock()
			if account.Home == other.Home {
				return ports.AgyAccountObservation{Authentication: domain.AgentAuthenticationAuthorized, Method: domain.AgyAuthMethodGoogle, Email: &otherEmail}, nil
			}
			if refresh && !refreshAuthorized {
				return ports.AgyAccountObservation{Authentication: domain.AgentAuthenticationUnauthorized, Method: domain.AgyAuthMethodUnknown}, nil
			}
			return ports.AgyAccountObservation{Authentication: domain.AgentAuthenticationAuthorized, Method: domain.AgyAuthMethodGoogle, Email: &activeEmail}, nil
		}, capacityFn: func(context.Context) (ports.AgyCapacityObservation, error) {
			if account.Home == other.Home {
				return ports.AgyCapacityObservation{}, nil
			}
			fixture.mu.Lock()
			...
			if index < len(fixture.activeCapacityErrors) {
				return ports.AgyCapacityObservation{}, fixture.activeCapacityErrors[index]
			}
			return ports.AgyCapacityObservation{}, nil
		}}, nil
	}}
	fixture.service = &Service{agyAccounts: manager, readiness: newReadinessCoordinator(readinessCoordinatorConfig{})}
	return fixture
}

func (f *agyLaunchReadinessFixture) signIn() {
	f.mu.Lock()
	f.refreshReturnsAuthorized = true
	f.mu.Unlock()
}

func (f *agyLaunchReadinessFixture) rejectActiveCapacity(errors ...error) {
	f.mu.Lock()
...
func (f *agyLaunchReadinessFixture) refreshCapableReads() int {
	f.mu.Lock()
	defer f.mu.Unlock()
	count := 0
...
func (f *agyLaunchReadinessFixture) protectedReads() int {
	f.mu.Lock()
	defer f.mu.Unlock()
	return f.activeCapacityReads
}

func (f *agyLaunchReadinessFixture) ensureSettings() AgyAccounts {
	f.t.Helper()
...
func (f *agyLaunchReadinessFixture) forceAuthenticationCheck() AgyAccounts {
	f.t.Helper()
	result, err := f.manager.ensure(context.Background(), []string{f.active.Snapshot.ID}, true, domain.AgentInstallationInstalled)
	if err != nil {
		f.t.Fatal(err)
	}
	return result
...
func (f *agyLaunchReadinessFixture) account(view AgyAccounts, id string) domain.AgyAccountSnapshot {
	f.t.Helper()
	...
	return domain.AgyAccountSnapshot{}
}

func TestAgySettingsUsesProtectedCallInsteadOfRefreshAccountRead(t *testing.T) {
	fixture := newAgyLaunchReadinessFixture(t)
...
func TestAgyLaunchFallsBackToNativeReadinessWhileDeviceReconciliationIsUnavailable(t *testing.T) {
	fixture := newAgyLaunchReadinessFixture(t)
	fixture.manager.mu.Lock()
	fixture.manager.reconciliation = domain.AgyDeviceReconciliation{
	...
	authentication, handled := fixture.service.structuredAgyAuthentication(
		context.Background(), string(domain.HarnessAgy), domain.AgentReadinessPurposeLaunch,
	)
	if handled {
...
func TestAgyLaunchFallsBackToNativeReadinessOnTransientProtectedFailure(t *testing.T) {
	fixture := newAgyLaunchReadinessFixture(t)
	fixture.rejectActiveCapacity(ports.ErrAgyCapacityProviderUnavailable)
	authentication, handled := fixture.service.structuredAgyAuthentication(context.Background(), string(domain.HarnessAgy), domain.AgentReadinessPurposeLaunch)
	if handled {
...
func TestAgyDisplayReadInFlightCannotClearARequiredReauthentication(t *testing.T) {
	...
	record := agyCommitTestAccount(t, manager.catalog, manager.pendingRoot, "b60a377d-da68-4a61-86f2-f31f04c571f2", ports.AgyAccountObservation{
		Authentication: domain.AgentAuthenticationAuthorized, Method: domain.AgyAuthMethodGoogle,
	})
...
func TestAgyAuthorizedInactiveAccountDoesNotMaskTheActiveAccount(t *testing.T) {
	fixture := newAgyLaunchReadinessFixture(t)
	fixture.rejectActiveCapacity(ports.ErrAgyOAuthTokenRevoked, ports.ErrAgyOAuthTokenRevoked)
	...
	authentication, ok := fixture.service.structuredAgyAuthentication(context.Background(), string(domain.HarnessAgy), domain.AgentReadinessPurposeDisplay)
	if !ok || authentication.State != domain.AgentAuthenticationUnauthorized {
		t.Fatalf("Agy readiness masked by the inactive account = %#v (structured=%t)", authentication, ok)
	}
...
func TestAgySuccessfulReauthenticationKeepsCapacitySeparateFromLaunchReadiness(t *testing.T) {
	fixture := newAgyLaunchReadinessFixture(t)
	...
	launch, ok := fixture.service.structuredAgyAuthentication(context.Background(), string(domain.HarnessAgy), domain.AgentReadinessPurposeLaunch)
	if ok {
		t.Fatalf("capacity success became a structured authentication decision = %#v", launch)
	}
}

func TestAgyRepeatedSettingsEnsuresReuseProtectedConfirmation(t *testing.T) {
	fixture := newAgyLaunchReadinessFixture(t)
...
func TestAgyUserAuthenticationRetryBypassesFreshCachesAndBackoff(t *testing.T) {
	fixture := newAgyLaunchReadinessFixture(t)
	fixture.ensureSettings()
	reads := fixture.protectedReads()
...
func TestAgyRotatedCredentialsDoNotReVerifyAnAuthorizedAccount(t *testing.T) {
	fixture := newAgyLaunchReadinessFixture(t)
	fixture.signIn()
	fixture.ensureSettings()
	reads := fixture.protectedReads()

	// A refresh-capable read can rotate Agy's device-global refresh token, so
...
func TestAgyExternallyReplacedCredentialsDoNotGetAttributedToOldActiveSlot(t *testing.T) {
	fixture := newAgyLaunchReadinessFixture(t)
	fixture.rejectActiveCapacity(ports.ErrAgyOAuthTokenRevoked, ports.ErrAgyOAuthTokenRevoked)
	view := fixture.ensureSettings()
	...
	original, err := readOpaqueCredential(filepath.Join(fixture.active.Home, agyCredentialFilename))
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := writeGlobalCredentialAtomic(fixture.manager.globalCredentialPath(), agyTestOAuthCredential("external-account", "replacement-access")); err != nil {
		t.Fatal(err)
	}
	...
	saved, err := readOpaqueCredential(filepath.Join(fixture.active.Home, agyCredentialFilename))
