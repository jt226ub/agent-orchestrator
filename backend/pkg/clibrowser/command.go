...
func NewCommand(t Transport, clock Clock, long string) *cobra.Command {
	...
	screenshot := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resp, err := t.BrowserAction(cmd.Context(), "screenshot", nil)
			if err != nil {
				return err
			}
			if screenshotBase64 {
			...
			return WriteScreenshot(cmd, resp.Result, path, jsonOutput)
		},
	}
...
func WriteScreenshot(cmd *cobra.Command, result map[string]any, target string, jsonOutput bool) error {
