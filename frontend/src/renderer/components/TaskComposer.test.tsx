...
describe("TaskComposer", () => {
	...
	it("waits for project context before allowing a local task to start", async () => {
		...
		h.get.mockImplementation(async (path: string) => {
			...
			// The default profiles are a separate read; only the project itself is
			// held pending here, or its resolver would be the one this test keeps.
	...
	it("preselects the worker profile, names the active template, and delegates with the profile", async () => {
		...
		// The composer names the visible selection explicitly, so the profile's own
		// model is what goes on the wire.
		expect(h.post.mock.calls[0][1].body).toMatchObject({ model: "flash-model" });
