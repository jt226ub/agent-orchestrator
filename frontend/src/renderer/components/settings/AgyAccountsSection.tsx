...
import { useAgyAccountActions } from "../../hooks/useAgyAccountActions";
import { agyAccountCanSwitch, agyAccountReasonKey, agyAuthenticationDisplay, agySwitchDisplay } from "../../hooks/agy-accounts-state";
import { useAgyAccountsQuery, useEnsureAgyAccounts, type AgyAccount, type AgyAccountSwitch, type AgyActiveLogin } from "../../hooks/useAgyAccountsQuery";
import { ConfirmDialog } from "../ConfirmDialog";
import { Button } from "../ui/button";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "../ui/dropdown-menu";
import { Tooltip, TooltipContent, TooltipTrigger } from "../ui/tooltip";
import { AgentProviderGroup } from "./AgentProviderGroup";
import { formatAuthMethod, formatPercentage } from "./AgyAccountDetails";
import { AgyAccountLoginTerminalPanel } from "./AgyAccountLoginTerminalPanel";
import { AgyAccountRow } from "./AgyAccountRow";
import { SettingsSection } from "./SettingsSection";

export type PendingAgyAccountAction =
	| { kind: "switch"; account: AgyAccount; idempotencyKey: string; submitting: boolean }
	| { kind: "delete"; account: AgyAccount; submitting: boolean }
	| null;

export function AgyAccountsSection({ titleHidden }: { titleHidden?: boolean }) {
	const { t } = useTranslation();
	const queryClient = useQueryClient();
	const accountsQuery = useAgyAccountsQuery();
	...
	useEnsureAgyAccounts();
	const actions = useAgyAccountActions(queryClient);
	const [providerExpanded, setProviderExpanded] = useState(true);
	const [expandedAccount, setExpandedAccount] = useState<string | null>(null);
	const [pendingAction, setPendingAction] = useState<PendingAgyAccountAction>(null);
	...
	const previousSwitch = useRef<AgyAccountSwitch | null>(null);
	...
	const activeAuthentication = activeAccount ? agyAuthenticationDisplay(activeAccount) : null;
	const currentSwitch = data?.currentSwitch;
	const switchPresentation = currentSwitch ? agySwitchDisplay(currentSwitch) : null;
	...
	const verifyLogin = useCallback(async (login: AgyActiveLogin) => {
		...
		setAnnouncement(t("settings.agyAccounts.loginSuccess", { label: operation.account.label }));
		window.requestAnimationFrame(() => document.getElementById(`agy-account-${operation.account?.id}`)?.focus());
	}, [actions, t]);

	const toggleAccount = useCallback((account: AgyAccount) => {
	...
	const openPending = (kind: Exclude<PendingAgyAccountAction, null>["kind"], account: AgyAccount) => {
	...
	const submitPending = useCallback(async () => {
		...
		try {
			switch (pending.kind) {
				...
				case "delete": await actions.deleteAccount(pending.account); if (expandedAccount === pending.account.id) setExpandedAccount(null); setAnnouncement(t("settings.agyAccounts.deleteSuccess", { label: pending.account.label })); break;
			}
	...
	const dialog = useMemo(() => {
		...
		switch (pendingAction.kind) {
			case "switch": return {
				title: t("settings.agyAccounts.switchTitle", { label: pendingAction.account.label }),
				description: t("settings.agyAccounts.switchDescription"),
				confirmLabel: t("settings.agyAccounts.switchConfirm"),
				destructive: false,
			};
			case "delete": return { title: t("settings.agyAccounts.deleteTitle"), description: t("settings.agyAccounts.deleteDescription", { label: pendingAction.account.label }), confirmLabel: t("settings.agyAccounts.delete"), destructive: true };
		}
	}, [pendingAction, t]);

	const summary = useMemo(() => {
		if (accountsError) return accountsError;
		if (!data) return t("settings.agyAccounts.loading");
		if (switchStatus && switchPresentation?.busy) return switchStatus;
		if (!deviceVerified) {
			if (deviceRefreshing && showDeviceRefresh) return t("settings.agyAccounts.reconciliationChecking");
			if (deviceBlocked) return t("settings.agyAccounts.reconciliationBlocked");
		}
		// The collapsed summary is all the user sees, so it must not read as ready
		// when the active account -- the one every Antigravity session launches with --
		// needs reauthentication, however healthy the other accounts look.
		if (activeAccount && activeAuthentication?.key !== "settings.agyAccounts.signedIn") return [activeAccount.label, t(activeAuthentication?.key ?? "settings.agyAccounts.authenticationCheckFailed")].join(" · ");
		if (activeAccount) return [activeAccount.label, activeAccount.capacity.remainingPercent == null ? null : `${formatPercentage(activeAccount.capacity.remainingPercent)} ${t("settings.agyAccounts.remaining")}`].filter(Boolean).join(" · ");
		if (deviceVerified && !data.activeAccountId) return data.accounts.length > 0
			? t("settings.agyAccounts.chooseAccount")
			: t("settings.agyAccounts.signInToUse");
		return t("settings.agyAccounts.count", { count: data.accounts.length });
	...
	return <SettingsSection title={t("settings.agyAccounts.title")} sectionId="agy-accounts" titleHidden={titleHidden}>
		<AgentProviderGroup provider="agy" name="Antigravity" summary={summary} expanded={providerHasContent && (providerExpanded || Boolean(activeLogin))} onExpandedChange={setProviderExpanded} collapsible={providerHasContent} collapseLocked={Boolean(activeLogin)} action={<div className="flex items-center gap-2">{switchPresentation?.busy && switchStatus ? <LoaderCircle className="size-5 animate-spin text-muted-foreground" aria-label={switchStatus} /> : null}{deviceBlocked && deviceRetryable ? <Button type="button" size="sm" variant="outline" disabled={actions.deviceRefreshPending} onClick={() => void actions.retryDeviceRefresh().catch(() => undefined)}>{actions.deviceRefreshPending ? <LoaderCircle className="animate-spin" aria-label={t("settings.agyAccounts.reconciliationChecking")} /> : null}{t("settings.agyAccounts.tryAgain")}</Button> : null}{switchSourceAvailable && switchTargets.length > 0 ? <DropdownMenu><DropdownMenuTrigger asChild><Button type="button" size="sm" variant="outline" aria-label={t("settings.agyAccounts.switchConfirm")} disabled={mutationDisabled || switchUnsupported} title={switchUnsupported && data ? t(agyAccountReasonKey(data.capabilities.globalSwitch.reasonCode)) : undefined}><ArrowRightLeft aria-hidden="true" />{t("settings.agyAccounts.switchAction")}</Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="min-w-64">{switchTargets.map((account) => { const targetSummary = formatAuthMethod(account.authMethod) ?? ""; return <DropdownMenuItem key={account.id} disabled={!agyAccountCanSwitch(account)} onSelect={() => openPending("switch", account)}><UserRound aria-hidden="true" /><div className="min-w-0"><p className="truncate text-foreground">{account.label}</p>{targetSummary ? <p className="truncate text-micro text-muted-foreground">{targetSummary}</p> : null}</div></DropdownMenuItem>; })}</DropdownMenuContent></DropdownMenu> : null}<Tooltip><TooltipTrigger asChild><Button type="button" size="icon-sm" variant="secondary" aria-label={t("settings.agyAccounts.add")} title={accountsError ?? undefined} onClick={() => void beginLogin()} disabled={mutationDisabled || data?.capabilities.nativeLogin.state !== "supported"}><Plus aria-hidden="true" /></Button></TooltipTrigger><TooltipContent>{t("settings.agyAccounts.add")}</TooltipContent></Tooltip></div>}>
			...
			{switchOutcome ? <p key={switchOutcome.switchId} className={`border-b border-border px-4 py-3 text-xs ${switchOutcome.result === "failed" || switchOutcome.result === "unchanged" ? "text-error" : "text-muted-foreground"}`} role="status" aria-live="polite">{t(`settings.agyAccounts.switch.${switchOutcome.result}`, { label: switchOutcome.label })}</p> : null}
			{activeLogin && !activeLogin.accountId ? <div className="border-b border-border px-4 py-3" data-testid="agy-account-pending-row"><AgyAccountLoginTerminalPanel activeLogin={activeLogin} pending={actions.loginOperationPending} onCheckAgain={() => void verifyLogin(activeLogin)} onClose={() => void actions.closeLogin(activeLogin)} onRetry={() => void actions.retryLogin(activeLogin)} /></div> : null}
			{accountsQuery.isLoading ? <p className="px-4 py-3 text-xs text-muted-foreground">{t("settings.agyAccounts.loading")}</p> : null}{accountsError ? <p className="px-4 py-3 text-xs text-error" role="alert">{accountsError}</p> : null}
			<div className="divide-y divide-border">{data?.accounts.map((account) => <AgyAccountRow key={account.id} account={account} expanded={expandedAccount === account.id} mutationDisabled={mutationDisabled} deviceMutationDisabled={mutationDisabled || switchUnsupported} canUse={!activeAccount && !deviceRefreshing && !switchUnsupported && agyAccountCanSwitch(account)} authenticationRetryBusy={actions.authenticationRetryAccountId === account.id} deleteBusy={pendingAction?.kind === "delete" && pendingAction.account.id === account.id && pendingAction.submitting} activeLogin={activeLogin?.accountId === account.id ? activeLogin : null} loginPending={actions.loginOperationPending} onToggle={() => toggleAccount(account)} onUseAccount={() => openPending("switch", account)} onRetryAuthentication={() => void actions.retryAuthentication(account.id).catch(() => undefined)} onSignIn={() => void beginLogin(account.id)} onDelete={() => openPending("delete", account)} onCheckLogin={() => activeLogin && void verifyLogin(activeLogin)} onCloseLogin={() => activeLogin && void actions.closeLogin(activeLogin)} onRetryLogin={() => activeLogin && void actions.retryLogin(activeLogin)} />)}</div>
		</AgentProviderGroup>
