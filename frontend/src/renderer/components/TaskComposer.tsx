...
export function TaskComposer({
	...
	const projectConfig = (isCloudProject ? cloudProject?.config : projectQuery.data?.config) as
		...
		| undefined;
	const projectWorkerAgent = selectedProfileConfig?.agent || projectConfig?.worker?.agent || "";
	...
	const defaultWorkerModel =
		selectedProfileConfig?.agentConfig?.model || projectConfig?.worker?.agentConfig?.model || projectConfig?.agentConfig?.model || "";
	const defaultWorkerMode =
		selectedProfileConfig?.agentConfig?.mode || projectConfig?.worker?.agentConfig?.mode || projectConfig?.agentConfig?.mode || "";
	const defaultWorkerEffort =
		selectedProfileConfig?.agentConfig?.effort || projectConfig?.worker?.agentConfig?.effort || projectConfig?.agentConfig?.effort || "";
